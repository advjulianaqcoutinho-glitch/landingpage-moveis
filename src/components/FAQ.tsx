...
function FAQItem({ q, a, index, inView }: { q: string; a: string; index: number; inView: boolean }) {
  ...
  return (
    <motion.div
      ...
      className={`border rounded-2xl overflow-hidden transition-all duration-300 ${open ? "border-[rgba(180,140,90,0.25)] bg-[rgba(180,140,90,0.03)]" : "border-[rgba(255,255,255,0.06)] bg-[rgba(255,255,255,0.015)] hover:border-[rgba(180,140,90,0.12)]"}`}
    >
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between gap-4 p-6 text-left">
        <span className={`font-medium leading-snug transition-colors duration-200 ${open ? "text-[#f0ede8]" : "text-[#a0a098]"}`}>{q}</span>
        <motion.div animate={{ rotate: open ? 45 : 0 }} transition={{ duration: 0.25 }} className={`flex-shrink-0 w-7 h-7 rounded-full border flex items-center justify-center transition-all duration-300 ${open ? "border-[rgba(180,140,90,0.4)] text-[#b48c5a]" : "border-[rgba(255,255,255,0.1)] text-[#4a4a48]"}`}>
          <Plus size={14} />
        </motion.div>
      </button>
      <AnimatePresence>
    ...
    <section ref={ref} className="py-32 px-6 relative overflow-hidden">
      ...
      <div className="relative max-w-3xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7 }} className="text-center mb-16">
          <span className="text-[#b48c5a] text-sm font-medium tracking-widest uppercase mb-4 block">Dúvidas frequentes</span>
          <h2 className="text-4xl md:text-5xl font-semibold text-[#f0ede8] leading-tight">Perguntas <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#c9a46e] to-[#8a6840]">respondidas</span></h2>
        </motion.div>
        <div className="space-y-3">
          {faqs.map((item, i) => <FAQItem key={i} q={item.q} a={item.a} index={i} inView={inView} />)}
        </div>
      </div>
    </section>
  );
